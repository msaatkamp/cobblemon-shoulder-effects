...
class LightSourceEffect : ShoulderEffect {
    class LightSourceShoulderStatusEffect(internal val pokemonIds: MutableList<UUID>) : StatusEffectInstance(StatusEffects.GLOWING, 2, 0, true, false, false) {
        ...
        override fun update(entity: LivingEntity, overwriteCallback: Runnable?): Boolean {
            ...
            if (duration == 10 * 20) { // 10 seconds remaining
                entity.sendMessage(Text.literal("Your glowing buff is about to wear off."))
            }
            if (cooldown == 20) { // 1 seconds to be ready
                entity.sendMessage(Text.literal("Your glowing buff is ready."))
            }
