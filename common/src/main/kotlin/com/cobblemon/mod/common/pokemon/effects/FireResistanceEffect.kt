...
class FireResistanceEffect : ShoulderEffect {
    class FireResistanceShoulderStatusEffect(internal val pokemonIds: MutableList<UUID>) : StatusEffectInstance(StatusEffects.FIRE_RESISTANCE, 2, 0, true, false, false) {
        ...
        override fun update(entity: LivingEntity, overwriteCallback: Runnable?): Boolean {
            ...
            if (duration == 10 * 20) { // 10 seconds remaining
                entity.sendMessage(Text.literal("Your fire resistance is about to wear off."))
            }
            if (cooldown == 20) { // 1 seconds to be ready
                entity.sendMessage(Text.literal("Your fire resistance is ready."))
            }
    ...
    override fun applyEffect(pokemon: Pokemon, player: ServerPlayerEntity, isLeft: Boolean) {
        ...
        if (effect != null) {
            ...
            player.addStatusEffect(FireResistanceShoulderStatusEffect(mutableListOf(pokemon.uuid)))
        }
    }
