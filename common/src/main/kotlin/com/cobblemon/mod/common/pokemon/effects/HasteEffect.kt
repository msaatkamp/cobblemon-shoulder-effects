...
class HasteEffect : ShoulderEffect {
    class HasteShoulderStatusEffect(val pokemonIds: MutableList<UUID>) :
        StatusEffectInstance(StatusEffects.HASTE, 2400, 1, true, false, false) {
        ...
        override fun update(entity: LivingEntity, overwriteCallback: Runnable?): Boolean {
            ...
            if (duration == 10 * 20) { // 10 seconds remaining
                entity.sendMessage(Text.literal("Your haste effect is about to wear off."))
            }
    ...
    override fun applyEffect(pokemon: Pokemon, player: ServerPlayerEntity, isLeft: Boolean) {
        ...
        if (effect != null && effect.cooldown == 0) {
            ...
            effect.cooldown = 20 * 300 // 1 minute cooldown
        } else if (effect == null) {
